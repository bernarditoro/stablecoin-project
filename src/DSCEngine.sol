...
contract DSCEngine is ReentrancyGuard {
    ...
    error DSCEngine__UnequalLengthForTokenAndPriceFeedAddresses();
    ...
    uint256 private constant LIQUIDATION_BONUS = 10; // Amount (in percentage) to go to liquidator as bonus for liquidating users below health factor
    ...
    constructor(address[] memory tokenAddresses, address[] memory priceFeedAddresses, address dscAddress) {
        if (tokenAddresses.length != priceFeedAddresses.length) {
            revert DSCEngine__UnequalLengthForTokenAndPriceFeedAddresses();
        }
        ...
        returns (uint256)
    {
        ...
        uint256 collateralAdjustedForThreshold = (collateralValueInUsd * 50) / 100; // You can only mint up to half of the USD equivalent of your collateral
    ...
    function getUsdValue(address token, uint256 amount) public view returns (uint256) {
        ...
        return ((uint256(price) * 1e10) * amount) / 1e18; // price returns 8 decimals and amount is 18 decimals
    }
    ...
    function getUserCollateralBalance(address user, address collateral) external view returns (uint256) {
